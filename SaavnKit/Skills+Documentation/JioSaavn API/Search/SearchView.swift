        ...
        NavigationStack {
            VStack(spacing: 0) {
                ZStack(alignment: .leading) {
                    let inputRange = searchFilter.indices.compactMap { CGFloat($0) }
                    let outputRange = searchFilter.compactMap { $0.size.width + 30 } // Add 30 for padding
                    let outputPositionRange = searchFilter.compactMap { $0.minX - 15 } // Adjust position by -15 to center it
                ...
                GeometryReader { geometry in
                    ...
                    .onChange(of: viewScrollState) { _, newValue in
                        if let newValue {
